...
def test_image_upload(tokens):
    ...
    with open(test_image_path, 'rb') as f:
        files = {'file': f}
        response = requests.post(f"{BASE_URL}{API_PREFIX}/upload-image", headers=headers, files=files)
...
def test_image_management(tokens, uploaded_filename=None):
    ...
    if response.status_code == 200:
        ...
        if images and uploaded_filename:
            ...
            response = requests.get(f"{BASE_URL}{API_PREFIX}/image/{uploaded_filename}", headers=headers)
...
def test_logout(tokens):
    ...
    print("\n7. Logging out...")
    response = requests.post(f"{BASE_URL}{API_PREFIX}/logout", headers=headers, json=logout_data)
